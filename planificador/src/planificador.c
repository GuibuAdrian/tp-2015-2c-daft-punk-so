...
void consola()
{
	...
	while(1)
	{
		...
		while(pch==NULL){
			...
			printf("Para consultas de comandos utilice el comando man \n");
			printf("Ingresar comando: ");
			fgets(comando, PACKAGESIZE, stdin);
			pch = strtok(comando,"\n");
			if(pch!=NULL)break;
		...
		{
			pch = strtok(NULL," \n");
			char mCod[PACKAGESIZE];
			while(pch==NULL){
				printf("Falta ingresar un mCod \n");
				printf("mCod: ");
				fgets(mCod, PACKAGESIZE, stdin);
				pch = strtok(mCod,"\n");
				if(pch!=NULL)break;
			}
		...
		else
		{
			if (strncmp(pch, "fz", 2) == 0) //Finalizar PID
			{
				...
				while(pch==NULL){
					printf("No se ha ingresado un PID \n");
					printf("PID: ");
